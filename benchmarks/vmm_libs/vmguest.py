...
import paramiko
import subprocess
from vmm_libs.cmdrunner import SSHCmdRunner, NativeCmdRunner
from vmm_libs.dut import DUT
from vmm_libs.vmimg import VMImage
from vmm_libs.vmm import VMMLibvirt
    ...
    def new_vm(
        ...
        if has_vtpm is True:
            if vtpm_path is None:
                vtpm_path = VTPM_PATH
            if vtpm_log is None:
                vtpm_log = f"{FRAMEWORK_LOG_DIR}/{vm_name}_vtpm_td.log"
...
class VirshSSH():
    ...
    def __init__(self,
        ...
        paramiko.util.log_to_file(filename=f'{FRAMEWORK_LOG_DIR}/{qemu_machine.name}-paramiko.log', level=logging.DEBUG)
